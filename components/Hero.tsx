...
export default function Hero() {
  const openAIAssistant = () => {
    window.dispatchEvent(new CustomEvent("open-docmaster-ai"));
  };
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-16 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
        <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="text-center lg:text-left">
          ...
          <div className="mt-12 grid grid-cols-2 gap-6 sm:grid-cols-4">
            <div><h3 className="text-3xl font-bold text-blue-600">20+</h3><p className="mt-1 text-sm text-gray-600">Tools</p></div>
            <div><h3 className="text-3xl font-bold text-blue-600">Smart</h3><p className="mt-1 text-sm text-gray-600">Workflow</p></div>
            <div><h3 className="text-3xl font-bold text-blue-600">100%</h3><p className="mt-1 text-sm text-gray-600">Secure</p></div>
            <div><h3 className="text-3xl font-bold text-blue-600">Fast</h3><p className="mt-1 text-sm text-gray-600">Processing</p></div>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="rounded-3xl border border-gray-200 bg-white p-6 shadow-2xl sm:p-8">
          <div className="mb-6 flex items-center justify-between"><div><p className="text-sm font-semibold uppercase tracking-wide text-blue-600">YAJU Field Tools</p><h3 className="mt-1 text-2xl font-bold text-gray-900">Popular Tools</h3></div><div className="rounded-2xl bg-blue-100 p-3 text-blue-600"><Sparkles size={24} /></div></div>
          <div className="space-y-4">
            <Link href="/tools/coordinates-converter" className="flex items-center justify-between rounded-xl bg-blue-50 p-4 transition hover:bg-blue-100"><span className="font-medium text-gray-800">📍 Coordinates Converter</span><span className="font-semibold text-green-600">✓ Ready</span></Link>
            ...
            <Link href="/tools/pdf/merge-pdf" className="flex items-center justify-between rounded-xl bg-blue-50 p-4 transition hover:bg-blue-100"><span className="font-medium text-gray-800">📄 Merge PDF</span><span className="font-semibold text-green-600">✓ Ready</span></Link>
